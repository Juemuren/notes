...
各个操作系统都会提供自己的原生 UI 组件，不同操作系统之间的接口通常不同。开发者为了应用能够跨平台，得维护逻辑几乎相同的多份代码，这很不方便。而 Web 能够很好地跨平台，于是有了 **使用 Web 技术开发移动应用和桌面应用** 的尝试。

有些人把这种新的应用形态叫做 _Hybrid 应用_，即混合应用；相对的，不使用 Web 技术的应用就叫做 _Native 应用_，即原生应用。

:::note[原生应用的定义]

本文对 **混合应用** 的定义为 **主要使用 Web 技术开发，并通过某种运行时、桥接层或原生宿主能力运行在桌面或移动操作系统上的应用**；而 **原生应用** 就是 **不依赖 Web 技术，并直接运行在桌面或移动操作系统上的应用**。

不过 **原生应用** 没有一个标准的定义。评价一个应用的 _原生程度_，通常可以参考以下的指标

- 原生 UI
  - 使用操作系统提供的 UI 组件：React Native / .NET MAUI
  - 使用自绘 UI：Qt / Flutter
  - 使用 Web UI：Cordova / Electron / Tauri
- 原生 API
  - 能够较自然地调用操作系统的 API：Qt / .NET MAUI
  - 调用 API 需要跨语言，但不需要跨进程：React Native / Flutter
  - 调用 API 存在 Web/IPC 边界：Cordova / Electron / Tauri
- 原生 Runtime
  - 不需要额外 Runtime，直接在操作系统上运行机器码：Qt
  - 需要语言 Runtime：React Native / .NET MAUI / Electron / Flutter
  - 需要 Web Runtime：Cordova / Electron / Tauri
