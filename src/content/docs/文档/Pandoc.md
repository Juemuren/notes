...
- **LaTeX** 推荐使用 `xelatex` 引擎，对中文、数学公式的支持不错，且编译速度不太慢。不过 LaTeX 坑有点多，安装麻烦，配置复杂，且默认的模板效果很差
- **Typst** 引擎指定 `typst` 就行，默认的排版和样式就很不错。
- **HTML** 引擎默认为 `weasyprint`，不过 `prince` 通常更好效果，但后者并不开源。pandoc 不支持使用 `chrome` 作为引擎。因此如果想使用 `chrome` 生成 **PDF**，需要先转为 **HTML**，再使用 [chrome-headless 的打印功能](../网络/Chrome.md#无头模式)。
