...
我按照用途对工具进行分类，但有些工具用处不止一种，且有些工具用处非常独特。因此下文的分类可能不够严谨。
...
- [fzf](../数据处理/Fzf.md) 同样提供了可用于交互式脚本 TUI 框架
...
- [prek](Prek.md) Git 钩子管理器。类似的工具还有 `lefthook`
- [hyperfine](Hyperfine.md) 命令基准测试工具，`time` 的升级版，用于比较不同命令的速度
- viddy 反复运行命令并监控输出，是更现代的 `watch`
- watchexec 监视文件，在其更改后自动重新运行命令
- checkexec 比较文件修改时间，若依赖文件比目标文件更新则运行命令

### 命令手册

用于查阅命令手册、纠正命令错误、查找命令历史或提供命令补全的工具

- [tldr](Tldr.md) 命令示例手册
- navi 命令备忘录，提供了基于 `fzf` 的 TUI
- thefuck 命令纠错工具
- Atuin 用 SQLite 保存命令历史记录，并提供工作目录 / 退出码 / 耗时等上下文
- [fzf](../数据处理/Fzf.md) 模糊查找命令历史
- carapace 跨平台、跨 Shell 的命令补全工具
- inshellisense 提供 IDE 式的 Shell 补全

### 文件目录

用于查找文件、管理文件资源或在目录间进行导航的工具

- [fd](Fd.md) 文件查找器，类似 `find` 但更现代
- [eza](Eza.md) 文件目录查看工具，类似 `ls` 但更现代
- yazi 文件管理器，有非常好用的 TUI
- f2 批量重命名文件
- ouch 用于压缩和解压文件或目录的命令行工具，支持所有主要格式
- upx 用于压缩可执行文件，并让其被压缩后仍然可以运行
- [zoxide](Zoxide.md) 快速工作目录跳转

### 终端美化

用于美化终端界面的工具。涵盖获取系统信息、修改提示符等各种功能

- WezTerm 功能强大的跨平台终端模拟器和多路复用器，由 Rust 实现
- zellij 更现代的 `tmux`，具有多终端面板，用于打造终端工作区
...
- gopass 密钥管理器。类似的工具还有 `vault`，后者功能更加强大，但也更加复杂
