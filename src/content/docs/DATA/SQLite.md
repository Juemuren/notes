...
该文件可以手写，也可以先用 MAA 的仓库识别功能得到 JSON，然后再编写脚本转为 CSV。脚本语言使用 _python_、_jq_ 甚至 _sql_ 都行。
...
另一种方式是将更新操作封装为一个脚本，使用 `bash`、`python` 等语言都可以。下面是用 [just](../CLI/Just.md) 的例子，任务写在 `Justfile` 里，使用 `just update <职业> <芯片类型> <增减量>` 执行更新操作
