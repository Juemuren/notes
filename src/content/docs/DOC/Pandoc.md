...
- **HTML** 引擎默认为 _weasyprint_。最大的优势是样式很丰富 ~~css 太强大了~~，但排版效果不太好。个人认为 pandoc 支持的引擎都不如 `chrome`，因此如果要通过 **HTML** 转 **PDF**，建议先转为 **HTML**，再使用 [chrome-headless 的打印功能](../NET/Chrome.md#无头模式) 生成 **PDF**
